...
contract Date {
    ...
    constructor(uint16 _year, uint8 _month, uint8 _day) {
     ...
     */
    function setDateFromYMD(uint16 _year, uint8 _month, uint8 _day) public {
        isoDate = DateUtils.convertYMDtoDateString(_year, _month, _day);
        isoDateTime = isoDate.toSlice().concat(" 00:00:00".toSlice());
        setYMDwithISO(isoDate);
        timestampDate = DateUtils.convertYMDtoTimestamp(_year, _month, _day);
     ...
     */
    function setDateTimeFromYMDHMS(uint16 _year, uint8 _month, uint8 _day, uint8 _hour, uint8 _minute, uint8 _second)
        public
    {
