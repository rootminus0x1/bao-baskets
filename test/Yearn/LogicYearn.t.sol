...
contract TestYearnLogicBackTest is Test, LogicYearnLUSD {
    ...
    function test_historicalRates() public {
        ...
        for (uint256 i = 0; i < timeSeries.length; i++) {
            ...
            Roller.rollForkToBlockContaining(vm, dt + 60 * 60, Roller.Bounds(0, 0, block.number, block.timestamp)); // add an hour
    ...
    function test_historicalRatesSnapshot() public {
        ...
        for (uint256 i = 0; i < timeSeries.length; i++) {
            ...
            uint256 dt = DateUtils.convertDateTimeStringToTimestamp(timeSeries[i].date);
            Roller.rollForkToBlockContaining(vm, dt + 60 * 60, Roller.Bounds(0, 0, lastBlock, lastTimestamp)); // add an hour
    ...
    function test_detailedhistoricalRates() public {
        ...
        uint256 lastBlock = block.number;
        uint256 lastTimestamp = block.timestamp;
        Roller.rollForkToBlockContaining(vm, finishTimestamp, Roller.Bounds(0, 0, lastBlock, lastTimestamp));
        uint256 finishBlock = block.number;
        Roller.rollForkToBlockContaining(vm, startTimestamp, Roller.Bounds(0, 0, lastBlock, lastTimestamp));
